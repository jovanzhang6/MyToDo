...
>
> 实施中修订（2026-09-25，业主授权「重写换角度」）：原「同窗口形态切换」在与 Windows 的对抗中四轮未收敛（DWM 系统圆角切角、Tauri 阴影玻璃框、大窗缩小边界补偿、WebView 重排时序），**重写为独立球窗口**：启动即创建 64×64 球窗（隐藏待命、resizable=false、shadow=false、DWM DONOTROUND），收球=主窗隐藏+球窗显示（就地出现），展开=球窗隐藏+主窗在球当前位置显示（尺寸取 pre_ball，出屏钳位）。两窗口尺寸终生不变，全部对抗性 bug 失去存在条件。球窗口用同前端（URL ?view=ball 识别），state-changed 广播双窗同步；get_state 的提醒检查跳过球窗（避免双窗并发双发通知）。
