...
> 实施中修订 ②（2026-09-25 深夜，业主确认统计后要求「设置界面也重新设计成整页面」）：设置由气泡升级为独立整页（外观滑杆 / 行为三开关：到期提醒·窗口置顶·开机自启 / 关于），三视图互斥切换归 views.ts；自启新增 set_autostart 命令与 StateDto.autostart_enabled（系统注册态为唯一事实源不变）。伴随缺陷修复：📊 双监听抵消、stats.ts 僵尸引用、HMR 断链改进程重启、报错条时间戳+自动清空。业主睡眠期间全部自主拍板见 docs/vibe/mytodo/decisions.md。
